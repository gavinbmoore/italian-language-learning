...
interface ReadingPracticeSessionProps {
  ...
  words: Array<{ word: string; translation: string; sentence: string }>;
  onComplete: (practiceResults: PracticeWord[]) => void;
}

// SRS Algorithm constants (matching backend)
...
export function ReadingPracticeSession({
  ...
  const [allCards, setAllCards] = useState<PracticeWord[]>([]);
  ...
  useEffect(() => {
    if (open && words.length > 0) {
      const initialCards: PracticeWord[] = words.map(w => ({
        word: w.word,
        translation: w.translation,
        sentence: w.sentence,
        learning_step: 0, // Start at step 0
      }));
  ...
  useEffect(() => {
    ...
    const handleKeyPress = (event: KeyboardEvent) => {
      ...
      // Rate card
      if (isFlipped) {
        const isLearning = (currentCard?.learning_step ?? 0) < 2;
  ...
  // Local SRS calculation (matches backend algorithm)
  const calculateNextReview = (quality: number, card: PracticeWord): { learning_step: number; in_session: boolean } => {
    ...
    if (learningStep < LEARNING_STEPS.length) {
      if (quality === 1) {
        ...
        learningStep = LEARNING_STEPS.length;
        inSession = false;
      }
    }

    return { learning_step: learningStep, in_session: inSession };
  };
  ...
  const handleReview = async (quality: number) => {
    ...
    // Calculate next review locally
    const result = calculateNextReview(quality, currentCard);
    const updatedCard = { ...currentCard, learning_step: result.learning_step };
    ...
    // Check if card should stay in session
    ...
    if (nextQueuedCard) {
      setSessionQueue(prev => prev.filter(q => q.card.word !== nextQueuedCard.card.word));
      setAllCards(prev => {
        const newCards = [...prev];
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>No Words to Practice</DialogTitle>
            <DialogDescription>
              You didn't select any unknown words during the reading.
            </DialogDescription>
          </DialogHeader>
  ...
  // Determine card direction based on learning step
  const isProduction = (currentCard?.learning_step ?? 0) >= 2;
  const frontText = isProduction ? currentCard?.translation : currentCard?.word;
  const backText = isProduction ? currentCard?.word : currentCard?.translation;
  const frontLabel = isProduction ? 'English' : 'Italian';
  const backLabel = isProduction ? 'Italian' : 'English';
      ...
      <DialogContent className="sm:max-w-3xl glass-card">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <DialogTitle className="text-2xl flex items-center gap-2">
                Reading Practice
                <Badge className="bg-gradient-to-r from-learning/20 to-learning/10 text-learning border-learning/30">
                  New Words
                </Badge>
              </DialogTitle>
              <DialogDescription className="mt-1">
                Practice your new vocabulary - they'll be added to Anki after this session
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {currentCard && (() => {
              const learningStep = currentCard.learning_step;
              if (learningStep < 2) {
                return (
                  <Badge className="bg-learning/10 text-learning border-learning/30">
                    📚 Recognition: Italian → English (Step {learningStep + 1}/2)
                  </Badge>
                );
              } else {
                return (
            ...
            {sessionQueue.length > 0 && (
              <Badge className="bg-due/10 text-due border-due/30">
                ⏳ {sessionQueue.length} to review again
              </Badge>
            )}
          </div>
        </div>
            ...
            >
              <CardContent className="flex flex-col items-center justify-center p-12 min-h-[350px]">
                <div className="inline-block px-4 py-1.5 rounded-full bg-primary/10 border border-primary/30 mb-6">
                  <p className="text-xs font-medium text-primary uppercase tracking-wide">{frontLabel}</p>
                </div>
              ...
              <CardContent className="flex flex-col items-center justify-center p-10 min-h-[350px]">
                <div className="text-center space-y-4 w-full">
                  ...
                  <div className="inline-block px-4 py-1.5 rounded-full bg-mastered/10 border border-mastered/30">
                    <p className="text-xs font-medium text-mastered uppercase tracking-wide">{backLabel}</p>
                  </div>
                  ...
                  {currentCard?.sentence && (
                    <div className="mt-6 p-5 bg-gradient-to-br from-muted/50 to-accent/30 rounded-xl border border-border/50">
                      <p className="text-xs font-medium text-primary mb-2 uppercase tracking-wide">Example from text</p>
                      <p className="italic text-base leading-relaxed italian-text text-foreground">{currentCard.sentence}</p>
                    </div>
                  )}
                </div>
              </CardContent>
        ...
        {isFlipped && (() => {
          ...
          if (isLearning) {
            ...
            return (
              <div className="space-y-4">
                <p className="text-center text-muted-foreground font-medium">
                  Can you produce the Italian word?
                </p>
